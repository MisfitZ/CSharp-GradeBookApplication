...
namespace GradeBook.GradeBooks
{
    public abstract class BaseGradeBook
    {
        ...
        {
            int addedWeightAmount = IsWeighted && (studentType == StudentType.DualEnrolled || studentType == StudentType.Honors) ? 1 : 0;

            switch (letterGrade)
            {
                case 'A':
                    return 4 + addedWeightAmount;
                case 'B':
                    return 3 + addedWeightAmount;
                case 'C':
                    return 2 + addedWeightAmount;
                case 'D':
                    return 1 + addedWeightAmount;
                case 'F':
                    return 0 + addedWeightAmount;
            }
            return 0;
        }
